     ...
     &     dist(maxpsrs),x(maxpsrs),y(maxpsrs),z(maxpsrs),pb,p1(maxpsrs)
      ...
      call getarg(1,cline)
      narg=iargc()
      if (cline.eq.''.or.narg.lt.2) stop 'usage: pkspsrs dmodel surveys'
      read(cline,*) dmod
      call glun(lun)
      ...
      do i=2,narg
         call getarg(i,cline)
         if (cline.eq.'PMSURV') then
            seed=system('psrcat -c 
     &      "gl gb p0 p1 w50 dm dist_dm s1400 pb survey name" -nonumber
     &       -o short -nohead|grep pksmb|sed s/"*"/0/g>>psrcat.in')
            pksmb=2**(i-2)
         elseif (cline.eq.'SWINMB1') then
            seed=system('psrcat -c 
     &      "gl gb p0 p1 w50 dm dist_dm s1400 pb survey name" -nonumber
     &       -o short -nohead|grep swmb|sed s/"*"/0/g>>psrcat.in')
            swmb=2**(i-2)
         elseif (cline.eq.'PHSURV') then
            seed=system('psrcat -c 
     &      "gl gb p0 p1 w50 dm dist_dm s1400 pb survey name" -nonumber
     &       -o short -nohead|grep phmb|sed s/"*"/0/g>>psrcat.in')
            phmb=2**(i-2)
         elseif (cline.eq.'SWINMB2') then
            seed=system('psrcat -c 
     &      "gl gb p0 p1 w50 dm dist_dm s1400 pb survey name" -nonumber
     &       -o short -nohead|grep swhi|sed s/"*"/0/g>>psrcat.in')
            swhi=2**(i-2)
         endif
      enddo
	...
	i=1
     ...
     &                dtrue(i),s(i),pb
	if (pb.eq.0.and.p0(i).gt.0.03) then
